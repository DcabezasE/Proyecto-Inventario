...
class GUI:
    ...
    def registrar_entrada(self):
        ...
        self.entradaframe.config(width="300", height="120")
    ...
    def registrar_salida(self):
        ...
        Label(self.entradaframe, text="Nombre del Producto:", fg="#9DF1FC", bg="#fca89d", font=(15)).grid(row=0, column=0)
        ...
        Button(self.entradaframe, text="Volver", command=self.volver).grid(row=2, column=0)
        Button(self.entradaframe, text="Añadir al carrito", command=self.carrito).grid(row=2, column=1)
        Button(self.entradaframe, text="Pagar", command=self.enviar_salida).grid(row=3, column=0)
    ...
    def carrito(self):
        ...
        for p in self.inventario.productos:
            if p.nombre == nombre:
                self.lista_venta.append([nombre, cantidad, p.precio])
    ...
    def resumen_de_venta(self):
        ...
        precio.insert(0, 'A pagar')

        for i,p in enumerate(self.lista_venta):
